...
def sh_order_df():
    return pd.DataFrame({
        ...
        "TickTime": ["091500040", "091501020"],  # 09:15:00.040 / 09:15:01.020
        "TickType": ["A", "D"],
        "Side": ["1", "2"],          # 注意：字符串
        "SubSeq": [1001, 1002],
        "Price": [10.2, 10.3],
...
def sh_trade_df():
    return pd.DataFrame({
        ...
        "TickTime": ["091505040"],  # 09:15:05.040
        "TickType": ["T"],
        "Side": ["1"],
        "SubSeq": [8888],
        "Price": [10.25],
...
def test_sh_order_parse():
    ...
    # ts 是 int（微秒）
    # assert isinstance(out["ts"].iloc[0], int)
    assert out["ts"].iloc[1] > out["ts"].iloc[0]
...
def test_sh_trade_parse():
    ...
    assert out["sell_no"].iloc[0] == 45002
...
def test_sz_order_parse():
    ...
    assert out["order_id"].iloc[0] == 6001
...
def test_sz_trade_parse():
    ...
    assert out["side"].iloc[0] is None


def test_event_time_order():
    """
    TickTime 决定 ts，排序应按 ts 数值
    """
    df = pd.DataFrame({
        "TradeTime": ["2025-11-07 09:00:00"] * 2,
        "TickTime": ["091500000", "091459999"],  # 第二条更早
        "TickType": ["A", "A"],
        "Side": ["1", "1"],
        "SubSeq": [1, 2],
        "Price": [10, 10],
