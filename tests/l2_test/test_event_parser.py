...
def sz_order_df():
    return pd.DataFrame({
        ...
        "TickTime": ["093000500"],  # 09:30:00.500
        "OrderType": [1],
        "Side": [1],
        "SubSeq": [6001],
        "Price": [9.85],
...
def test_sh_order_parse():
    ...
    out = parse_events(df, exchange_id=1, kind="order")
    ...
    assert out["ts"].iloc[0].microsecond == 40000
...
def test_sh_trade_parse():
    ...
    out = parse_events(df, exchange_id=1, kind="trade")
...
def test_sz_order_parse():
    df = sz_order_df()
    out = parse_events(df, exchange_id=2, kind="order")
...
def test_sz_trade_parse():
    df = sz_trade_df()
    out = parse_events(df, exchange_id=2, kind="trade")
...
def test_event_time_order():
    ...
    out = parse_events(df, 1, "order").sort_values("ts")
